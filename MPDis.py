...
newLayer = compute_new_layer(eInPI, distPhi, layerNum, paraLayerGen, shapeLayerGen, bead)
...
# @jax.jit
def compute_load_vector(newLayer, i, j, xTopo, subsFilter, elements, coords, Emat, Emin, nu):
    # Heat flux load
    ...
    Connectivity = jnp.array([[2*elements[:,0]],[2*elements[:,0] + 1],
                                   [2*elements[:,1]],[2*elements[:,1] + 1],
                                   [2*elements[:,2]],[2*elements[:,2] + 1]])
    ...
    dummyNewLayerKron = jnp.kron(jnp.ravel(dummyNewLayer) ** penalQ, jnp.ones((load_ihs.shape[0],))).reshape(-1, 1)
    ...
    return dummyLoad, dummyLoad1, dummyLoad2, dummyLoad3, dummyLoad4, dummyLoad5, dummyLoad6 
...
fixed_dofs = jnp.concatenate([subsNodeIndex * dofs_per_node, (subsNodeIndex * dofs_per_node)+1])
...
# ---------------------------------------------------------------------------
csr_full = sparse.create_sparsity_pattern(mesh, n_dofs_per_node=2)
...
reduced_sparsity_pattern = sparse.reduce_sparsity_pattern(csr_full, lifter.free_dofs)
...
def total_energy(u_free: Array, Epenal) -> Array:
    """Compute the total energy of the system."""
    u_full = jnp.zeros(n_dofs).at[free_dofs].set(u_free.ravel())
...
@jax.jit
def fn(u_free: Array, Epenal) -> Array:
    res = residual(u_free, Epenal)
    return f_ext - res

@jax.jit
def compute_tangent(du, u_prev):
    tangent = jax.jvp(fn, (u_prev,Epenal), (du,jnp.zeros_like(Epenal)))[1]
    return tangent
...
for i in range(1, numSubParts + 1):
    for j in range(60, maxNumLayers + 1):
        ...
        f_ext_full, f1, f2, f3, f4, f5, f6 = compute_load_vector(newLayer, i-1, j-1, xTopo, subsFilter, elements, coords,Emat, Emin, nu)
        f_ext = f_ext_full.at[free_dofs].get()

        u_sol, norm_res = newton_krylov_solver(
            u = jnp.zeros_like(f_ext),
            gradient = lambda u: fn(u, Epenal),
